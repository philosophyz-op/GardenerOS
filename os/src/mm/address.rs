...
impl From<VirtAddr> for VirtPageNum {
    fn from(v: VirtAddr) -> Self { assert_eq!(v.page_offset(), 0); v.floor() }
}
...
impl From<PhysAddr> for PhysPageNum {
    fn from(v: PhysAddr) -> Self { assert_eq!(v.page_offset(), 0); v.floor() }
}
...
impl<T> SimpleRange<T> where T: StepByOne + Copy + PartialEq + PartialOrd + Debug {
    pub fn new(start: T, end: T) -> Self { assert!(start <= end); Self { l: start, r: end } }
...
impl<T> IntoIterator for SimpleRange<T> where T: StepByOne + Copy + PartialEq + PartialOrd + Debug {
    type Item = T; type IntoIter = SimpleRangeIterator<T>;
...
impl Debug for VirtAddr { fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result { write!(f, "VA:{:#x}", self.0) } }
impl Debug for VirtPageNum { fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result { write!(f, "VPN:{:#x}", self.0) } }
impl Debug for PhysAddr { fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result { write!(f, "PA:{:#x}", self.0) } }
impl Debug for PhysPageNum { fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result { write!(f, "PPN:{:#x}", self.0) } }